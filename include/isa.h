...
typedef struct {
    uint32_t encoded;  // 32-bit encoded instruction
    uint32_t pc;       // program counter
    int valid;         // 1 = valid, 0 = bubble/invalid
} EncodedInst;

// Opcode field extraction (bits 26-31, 6 bits)
...
// Immediate field mask (bits 0-10, 11 bits, sign-extended)
#define IMM_MASK     0x7FF
#define IMM_SIGN_BIT 0x400

// Function to encode instruction as 32-bit value
static inline uint32_t encode_instruction(Opcode op, int rd, int rs1, int rs2, int32_t imm) {
    uint32_t instr = 0;
    
    // Place opcode in bits 26-31
    instr |= ((op & OPCODE_MASK) << OPCODE_SHIFT);
    
    // Place registers (handle rd=-1 for no destination)
    if (rd >= 0) instr |= ((rd & REG_MASK) << RD_SHIFT);
    if (rs1 >= 0) instr |= ((rs1 & REG_MASK) << RS1_SHIFT);
    if (rs2 >= 0) instr |= ((rs2 & REG_MASK) << RS2_SHIFT);
    
    // Place immediate (11 bits, lower bits)
    instr |= (imm & IMM_MASK);
...
int execute_instruction(IDEXreg* input, ProgramCounter pc, LabelEntry label_table);
int add(int a, int b);
