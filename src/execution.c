...
static ExecutionResult* execute_single_cycle(InstMem *im, LabelEntry labels[] __attribute__((unused)),
...
static ExecutionResult* execute_pipelined(InstMem *im, LabelEntry labels[] __attribute__((unused)),
                                          int label_count __attribute__((unused)), Framebuffer *fb) {
    ExecutionResult *result = (ExecutionResult*)malloc(sizeof(ExecutionResult));
    if (!result) return NULL;
    ...
    printf("5-stage pipeline: IF → ID → EX → MEM → WB\n\n");
