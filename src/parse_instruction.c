...
void detectLabels(const char *filename, LabelEntry label_table[],
                  int *label_index) {
...
void instruction_parser(IFIDreg *ifid, DecodedInst *out) {
  ...
  while (token != NULL) {
    if (is_label(token)) {
      ...
      out->rs1 = parse_register(rs1);
      out->rs2 = -1;
      out->op = (strcmp(token, "SIN") == 0) ? OP_SIN : OP_COS;
      out->imm = 0;
      out->valid = (out->rd >= 0 && out->rs1 >= 0);
      return;
    }

    else if (strcmp(token, "MOVETO") == 0 || strcmp(token, "LINETO") == 0) {
      /* MOVETO rs_x, rs_y */
      char *rx = strtok_r(NULL, delimiters, &saveptr);
      char *ry = strtok_r(NULL, delimiters, &saveptr);

      out->rs1 = parse_register(rx);
      out->rs2 = parse_register(ry);
      out->op = (strcmp(token, "MOVETO") == 0) ? OP_MOVETO : OP_LINETO;
      out->rd = -1;
      out->imm = 0;
    ...
    if (strcmp(token, "LW") == 0) {
      ...
      if (!rest)
        rest = strtok_r(NULL, delimiters, &saveptr);
    ...
    if (strcmp(token, "BEQ") == 0) {
      ...
      out->imm = parse_immediate(offt); /* assembler should produce numeric PC
...
int parse_register(const char *token) {
  ...
  if (token[0] == 'x' || token[0] == 'X' || token[0] == 'r' ||
      token[0] == 'R') {
    return atoi(token + 1);
  }
  return -1; // invalid
